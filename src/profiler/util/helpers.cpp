...
namespace util
{
    ...
    HRESULT GetObjectTypeRef(util::ComPtr< IMetaDataEmit2>& metadataEmit, util::ComPtr<IMetaDataAssemblyEmit>& metadataAssemblyEmit, mdToken* objectTypeRef)
    {
        ...
        if (FAILED(hr))
        {
            logging::log(logging::LogLevel::PROFILERERROR, "Failed GetObjectTypeRef GetMsCorLibRef"_W);
            return hr;
        }
        ...
        if (FAILED(hr))
        {
            logging::log(logging::LogLevel::PROFILERERROR, "Failed GetObjectTypeRef DefineTypeRefByName"_W);
            return hr;
        }
